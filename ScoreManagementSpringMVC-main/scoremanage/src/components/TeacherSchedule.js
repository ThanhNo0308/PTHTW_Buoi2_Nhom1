...
import { Container, Table, Card, Row, Col, Form, Alert, Spinner, Badge, Button } from 'react-bootstrap';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faCalendarAlt, faInfoCircle, faBook, faClock, faUser, faDoorOpen, faDownload, faFilter } from '@fortawesome/free-solid-svg-icons';
...
const TeacherSchedule = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                setError(err.response?.data?.error || "Không thể tải lịch dạy. Vui lòng thử lại sau.");
            } finally {
                setLoading(false);
            }
        };
    ...
    // Load lịch dạy khi thay đổi học kỳ
    const loadSchedule = async (schoolYearId) => {
        try {
            setLoading(true);
            setError(null);

            const response = await scheduleApis.getTeacherSchedule(schoolYearId);

            if (response.data.success) {
                // Nhóm các buổi dạy theo ngày trong tuần
                const sortedSessions = groupSessionsByDayOfWeek(response.data.classSessions);
                setSchedule(sortedSessions);
                setTeacher(response.data.teacher);
            } else {
                setError(response.data.error || "Không thể tải lịch dạy");
            }
        } catch (err) {
            console.error("Error loading schedule:", err);
            setError(err.response?.data?.error || "Không thể tải lịch dạy. Vui lòng thử lại sau.");
            ...
            <Card className="shadow-sm mb-4">
                <Card.Header className="bg-primary text-white">
                    <div className="d-flex justify-content-between align-items-center">
                        <Card.Title className="mb-0">Thông tin lịch dạy</Card.Title>
                    ...
                    {loading ? (
                        ...
                        <>
                                    ...
                                    <Col md={6}>
                                        <Card className="shadow-sm">
                                            <Card.Body>
                                                ...
                                                <p><strong>Họ và tên:</strong> {teacher.teacherName}</p>
                                                <p><strong>Khoa:</strong> {teacher.department}</p>
                                            </Card.Body>
                                        </Card>
                                    </Col>
                                    <Col md={6}>
                            ...
                            <Alert variant="info" className="mt-3">
                                <FontAwesomeIcon icon={faInfoCircle} className="me-2" />
                                <strong>Lưu ý:</strong> Lịch dạy có thể thay đổi. Vui lòng kiểm tra thường xuyên và luôn theo dõi thông báo từ Phòng Đào tạo.
                            </Alert>
                        </>
                    )}
                </Card.Body>
...
// Component hiển thị một buổi dạy
    ...
    return (
        <div className="schedule-item mb-2 p-2" style={{
            ...
            <div>
                ...
                Lớp: {session.subject.className || "Chưa phân lớp"}
            </div>
            <div>
